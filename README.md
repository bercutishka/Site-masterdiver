...
| `blog/<slug>/index.html` | Сгенерированные пререндер-страницы статей (мета + текст в статике, для SEO/превью). Делает `npm run build`; коммитятся; GitHub Pages раздаёт. |
| `scripts/prerender.mjs` | Генератор пререндер-страниц из массива `posts`. |
...
- **Фронтенд → GitHub Pages.** Автоматически из ветки `main`. Прямые ссылки на статьи
  обслуживают **пререндер-страницы** `blog/<slug>/index.html` (коммитятся в репо — Pages
  собственный билд не запускает); прочие неизвестные пути ловит `404.html`.
  При изменении статей: `npm run build` → закоммитить `blog/`.
...
npm run build        # сгенерировать пререндер-страницы статей (blog/<slug>/index.html)
...
2. ✅ **Пререндер статей** — сделано: `npm run build` генерирует `blog/<slug>/index.html`
   с мета-тегами и текстом статьи. Прямой заход и боты-превью соцсетей видят контент;
   страницы коммитятся, Pages раздаёт. (При изменении статьи нужно пересобрать.)
3. ✅ **Вынос base64-картинок в файлы** — сделано: 8 фото в `img/`, `index.html` 1.2 МБ → ~160 КБ, `loading="lazy"`.
4. ✅ **Доступность** — сделано: `aria-expanded`/`aria-controls` на бургере, `role="log"`+`aria-live`
   в чате, `role="dialog"`+`aria-hidden` на панели, перевод фокуса на заголовок экрана в `go()`.
