...
| `blog/<slug>/index.html` | **Сгенерированные** пререндер-страницы статей (мета + текст в статике). Делает `npm run build` (`scripts/prerender.mjs`) из массива `posts`. Коммитятся; Pages раздаёт. Воркеру не отдаём (в `.assetsignore`). |
| `scripts/prerender.mjs` | Генератор пререндер-страниц. Экспортирует `generate()` — её же использует `npm run check` для проверки актуальности. |
